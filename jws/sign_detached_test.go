...
func TestSignDetachedReader(t *testing.T) {
	...
	t.Run("RoundTrip", func(t *testing.T) {
		t.Run("HS256", func(t *testing.T) {
			...
			require.NoError(t, err, `jws.SignDetachedReader should succeed`)
		...
		t.Run("PS256", func(t *testing.T) {
			...
			signed, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.PS256(), privkey))
			require.NoError(t, err, `jws.SignDetachedReader should succeed`)
		...
		t.Run("ES256", func(t *testing.T) {
			...
			signed, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.ES256(), privkey))
			require.NoError(t, err, `jws.SignDetachedReader should succeed`)
	...
	t.Run("EdDSA returns error", func(t *testing.T) {
		...
		_, err = jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.EdDSA(), privkey))
		require.Error(t, err, `jws.SignDetachedReader should fail for EdDSA`)
	})
	...
	t.Run("b64=false", func(t *testing.T) {
		...
		signed, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.HS256(), key, jws.WithProtectedHeaders(hdrs)))
		require.NoError(t, err, `jws.SignDetachedReader should succeed`)
	...
	t.Run("IO error mid-stream", func(t *testing.T) {
		...
		require.Error(t, err, `jws.SignDetachedReader should fail on I/O error`)
	})
	...
	t.Run("Cross-verify with jws.Sign", func(t *testing.T) {
		...
		for _, tc := range algorithms {
			t.Run(tc.alg.String(), func(t *testing.T) {
				// Sign with jws.Sign, verify with jws.VerifyDetachedReader
				signedTraditional, err := jws.Sign(nil, jws.WithKey(tc.alg, tc.privkey), jws.WithDetachedPayload(payload))
				require.NoError(t, err, `jws.Sign should succeed`)

				err = jws.VerifyDetachedReader(signedTraditional, bytes.NewReader(payload), jws.WithKey(tc.alg, tc.pubkey))
				require.NoError(t, err, `jws.VerifyDetachedReader should verify jws.Sign output`)

				// Sign with jws.SignDetachedReader, verify with jws.Verify
				signedStreaming, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(tc.alg, tc.privkey))
				require.NoError(t, err, `jws.SignDetachedReader should succeed`)

				_, err = jws.Verify(signedStreaming, jws.WithKey(tc.alg, tc.pubkey), jws.WithDetachedPayload(payload))
				require.NoError(t, err, `jws.Verify should verify jws.SignDetachedReader output`)
			})
		}
	})
	...
	t.Run("Rejects WithDetachedPayload option", func(t *testing.T) {
		key := jwxtest.GenerateSymmetricKey()
		_, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.HS256(), key), jws.WithDetachedPayload(payload))
	...
	t.Run("Rejects WithBase64Encoder option", func(t *testing.T) {
		key := jwxtest.GenerateSymmetricKey()
		_, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.HS256(), key), jws.WithBase64Encoder(base64.RawURLEncoding))
	...
	t.Run("Rejects multiple keys", func(t *testing.T) {
		...
		_, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.HS256(), key1), jws.WithKey(jwa.HS256(), key2))
	...
	t.Run("jwk.Key input", func(t *testing.T) {
		...
		signed, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(jwa.RS256(), privkey))
		require.NoError(t, err, `jws.SignDetachedReader with jwk.Key should succeed`)

		err = jws.VerifyDetachedReader(signed, bytes.NewReader(payload), jws.WithKey(jwa.RS256(), pubkey))
		require.NoError(t, err, `jws.VerifyDetachedReader with jwk.Key should succeed`)
	})
	...
	t.Run("JSON output round-trip", func(t *testing.T) {
		...
		for _, tc := range algorithms {
			t.Run(tc.alg.String(), func(t *testing.T) {
				signed, err := jws.SignDetachedReader(bytes.NewReader(payload), jws.WithKey(tc.alg, tc.privkey), jws.WithJSON())
				require.NoError(t, err, `jws.SignDetachedReader with WithJSON should succeed`)
				...
				require.NoError(t, err, `jws.VerifyDetachedReader should auto-detect JSON input`)

				err = jws.VerifyDetachedReader(signed, bytes.NewReader(payload), jws.WithKey(tc.alg, tc.pubkey), jws.WithJSON())
				require.NoError(t, err, `jws.VerifyDetachedReader with explicit WithJSON should succeed`)
			})
		}
	})
