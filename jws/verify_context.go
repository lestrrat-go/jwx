	...
	for _, option := range options {
		...
		case identMessage{}:
			if err := option.Value(&vc.dst); err != nil {
				return errFromVerify(`invalid value for option WithMessage: %w`, err)
			}
		case identDetachedPayload{}:
			if err := option.Value(&vc.detachedPayload); err != nil {
				return errFromVerify(`invalid value for option WithDetachedPayload: %w`, err)
			}
		case identKey{}:
			var pair *withKey
			if err := option.Value(&pair); err != nil {
				return errFromVerify(`invalid value for option WithKey: %w`, err)
			}
		...
		case identKeyProvider{}:
			...
			if err := option.Value(&kp); err != nil {
				return errFromVerify(`failed to retrieve key-provider option value: %w`, err)
			}
			vc.keyProviders = append(vc.keyProviders, kp)
		case identKeyUsed{}:
			if err := option.Value(&vc.keyUsed); err != nil {
				return errFromVerify(`failed to retrieve key-used option value: %w`, err)
			}
		case identContext{}:
			if err := option.Value(&vc.ctx); err != nil {
				return errFromVerify(`failed to retrieve context option value: %w`, err)
			}
		case identValidateKey{}:
			if err := option.Value(&vc.validateKey); err != nil {
				return errFromVerify(`failed to retrieve validate-key option value: %w`, err)
			}
		case identSerialization{}:
			vc.parseOptions = append(vc.parseOptions, option.(ParseOption))
		case identBase64Encoder{}:
			if err := option.Value(&vc.encoder); err != nil {
				return errFromVerify(`failed to retrieve base64-encoder option value: %w`, err)
			}
		default:
			return errFromVerify(`invalid jws.VerifyOption %q passed`, `With`+strings.TrimPrefix(fmt.Sprintf(`%T`, option.Ident()), `jws.ident`))
		}
	}

	if len(vc.keyProviders) < 1 {
		return errFromVerify(`no key providers have been provided (see jws.WithKey(), jws.WithKeySet(), jws.WithVerifyAuto(), and jws.WithKeyProvider()`)
	}
...
func (vc *verifyContext) VerifyMessage(buf []byte) ([]byte, error) {
	...
	if err != nil {
		return nil, errFromVerify(`failed to parse jws: %w`, err)
	}
	defer msg.clearRaw()

	if vc.detachedPayload != nil {
		if len(msg.payload) != 0 {
			return nil, errFromVerify(`can't specify detached payload for JWS with payload`)
		}
	...
	for idx, sig := range msg.signatures {
		...
		if rawHeaders == nil {
			...
			if err != nil {
				return nil, errFromVerify(`failed to marshal "protected" for signature #%d: %w`, idx+1, err)
			}
		...
		for i, kp := range vc.keyProviders {
			...
			if err := kp.FetchKeys(vc.ctx, &sink, sig, msg); err != nil {
				return nil, errFromVerify(`key provider %d failed: %w`, i, err)
			}
			...
			for _, pair := range sink.list {
				...
				if err := vc.tryKey(verifyBuf, alg, key, msg, sig); err != nil {
					errs = append(errs, errFromVerify(`failed to verify signature #%d with key %T: %w`, idx+1, key, err))
					continue
				}
		...
		errs = append(errs, errFromVerify(`signature #%d could not be verified with any of the keys`, idx+1))
	}
	return nil, errFromVerify(`could not verify message using any of the signatures or keys: %w`, errors.Join(errs...))
}
...
func (vc *verifyContext) tryKey(verifyBuf []byte, alg jwa.SignatureAlgorithm, key any, msg *Message, sig *Signature) error {
	...
	if err := verifier.Verify(key, verifyBuf, sig.signature); err != nil {
		return errFromVerification("%w", err)
	}
