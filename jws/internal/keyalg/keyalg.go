...
// nil if no key type is registered for alg (e.g. an unknown algorithm from
// an extension that isn't loaded), which signals callers to skip any
...
func Candidates(key any) ([]jwa.SignatureAlgorithm, error) {
	...
	case ed25519.PublicKey, ed25519.PrivateKey:
		// Candidates classifies by key type to report which algorithms a
