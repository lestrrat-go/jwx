...
//
// `jws.WithKey()` can further accept sub-options to change signing behavior
...
//
// These sub-options are ignored when the `jws.WithKey()` option is used with `jws.Verify()`.
...
// the sub-option `WithKeySetRequired` via `jws.WithKey(key, jws.WithRequireKid(false))`.
