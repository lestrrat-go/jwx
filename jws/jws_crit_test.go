...
//
// v3 note: the check is gated by critValidation (default false on v3, true
// on v4). The test passes WithCritValidation(true) explicitly so the v3
// backport asserts the same behavior as v4's default-strict path.
func TestVerifyRejectsB64FalseWithoutCrit(t *testing.T) {
	...
	_, err := jws.Verify(compact,
		jws.WithKey(jwa.HS256(), rawKey),
		jws.WithCritValidation(true),
	)
