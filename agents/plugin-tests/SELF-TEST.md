...
- "My token has no `kid`, but my JWKS contains exactly one key. How should I verify it?" — JWK Set section.
- "How do I reject an ES256 signature made with the wrong elliptic curve?" — JWS section.
- "Why did `jwk.Parse` accept a JWKS containing an unknown key type?" — JWK Set section.
