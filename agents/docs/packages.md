...
- **AlgorithmsForKey()** — DEPRECATED, do not use and do not add callers. It was never meant to be consumed by end users; it is jwx's internal algorithm-inference helper, exported for historical reasons only. The implementation lives in `jws/internal/keyalg` (`keyalg.Candidates`), which every in-tree caller uses directly. It is not a key/algorithm compatibility check. Compatibility is kept for the rest of v4 and it goes away in the next major version; until then it receives no fixes and its classification rule must not move (the result still widens when an extension registers, since the tables are an input to the rule, not part of it). Do NOT patch it. It is a one-line delegation only because `keyalg.Candidates` still classifies exactly as it did before deprecation — when that changes, give `AlgorithmsForKey` a frozen copy of the old rule rather than letting the change reach it.
