...
- **Fetcher** — transport-neutral interface implemented by companion modules such as
...
- Options: `WithKey()`, `WithKeySet()`, `WithVerifyAuto()`, `WithJSON()`, `WithDetachedPayload()`, `WithDetachedPayloadReader()` (streaming variant; single-key, HMAC/RSA/ECDSA only), `WithStrictECDSA()` (sign-only; see ECDSA curve binding below)
