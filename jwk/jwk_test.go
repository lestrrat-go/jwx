...
func TestGH567(t *testing.T) {
  ...
  "keys": [
	...
	ar.Configure(srv.URL, jwk.WithPreprocessor(jwk.ProcessFunc(func(in io.Reader) (io.Reader, error) {
		var m map[string]interface{}
		if err := json.NewDecoder(in).Decode(&m); err != nil {
			return nil, err
		}

		rkeys := m["keys"].([]interface{})
		for i, rkey := range rkeys {
			key := rkey.(map[string]interface{})
			if key["kid"] == "A2E10A6BAF4E43E86273F57F218A44B824203176" {
				m["keys"] = append(rkeys[:i], rkeys[i+1:]...)
				break
			}
		}
