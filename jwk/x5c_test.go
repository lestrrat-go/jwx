...
func Test_X5CHeader(t *testing.T) {
	t.Run("X509CertChain", func(t *testing.T) {
		...
		if !assert.NoError(t, h.Set(jwk.X509CertChainKey, certs), "Set for x5c should succeed") {
			return
		}

		_, ok := h.Get(jwk.X509CertChainKey)
		if !assert.True(t, ok, "Get for x5c should succeed") {
			return
		}

		asMap := map[string]interface{}{}
		err := h.PopulateMap(asMap)
		if !assert.NoError(t, err, "PopulateMap should succeed") {
			return
		}

		t.Logf("Map: %v", asMap)
		assert.Equal(t, certs, asMap[jwk.X509CertChainKey])
	})
}
