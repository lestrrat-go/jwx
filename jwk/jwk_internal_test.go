	...
	for _, key := range []Key{
		...
		t.Run("Set X509CertChainKey", func(t *testing.T) {
			t.Run("string", func(t *testing.T) {
				if !assert.NoError(t, key.Set(X509CertChainKey, certs[0]), "Set for x5c should succeed") {
					return
				}

				gotcerts, ok := key.Get(X509CertChainKey)
			...
			t.Run("[]string", func(t *testing.T) {
				if !assert.NoError(t, key.Set(X509CertChainKey, certs), "Set for x5c should succeed") {
					return
				}
			...
			t.Run("[]interface{} with string elements", func(t *testing.T) {
				tmp := make([]interface{}, len(certs))
				for i := 0; i < len(certs); i++ {
					tmp[i] = certs[i]
				}

				if !assert.NoError(t, key.Set(X509CertChainKey, tmp), "Set for x5c should succeed") {
					return
				}
