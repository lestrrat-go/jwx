...
func TestIsPrivate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			k:         &symmetricKey{},
			isPrivate: false,
			err:       "is not an asymmetric key",
		},
	}

	for _, tt := range cases {
		result, err := IsPrivate(tt.k)
		if tt.err == "" {
			assert.NoError(t, err, `%T should succeed`, tt.k)
		} else {
			assert.ErrorContains(t, err, tt.err)
		}
		if tt.isPrivate {
			assert.True(t, result, `%T should be a private key`, tt.k)
		} else {
			assert.False(t, result, `%T should be a public key`, tt.k)
		}
	}
