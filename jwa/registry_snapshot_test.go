...
func testRegistrySnapshot[T interface {
	...
	for round := range rounds {
		t.Run(fmt.Sprintf("%s/round-%02d", tc.name, round), func(t *testing.T) {
			...
			tc.unregister(values...)
			runConcurrentRegistryMutation(t, tc.register, values)
			assertRegistryState(t, tc, values, true)

			runConcurrentRegistryRemoval(t, tc.unregister, values)
			assertRegistryState(t, tc, values, false)
		})
	}
}

func runConcurrentRegistryMutation[T any](t *testing.T, register func(...T) error, values []T) {
	t.Helper()
...
func runConcurrentRegistryRemoval[T any](t *testing.T, unregister func(...T), values []T) {
	t.Helper()
