...
func testBuiltinRegistryProtection[T interface {
	...
	t.Run(tc.name, func(t *testing.T) {
		...
		err := tc.register(orig)
		require.NoError(t, err, "%s Register must allow idempotent builtin registration", tc.name)

		err = tc.register(tc.replacement(orig.String()))
		require.Error(t, err, "%s Register must reject overriding a builtin identifier", tc.name)
