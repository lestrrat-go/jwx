...
func TestJoseCompatibility(t *testing.T) {
	...
	t.Run("jwe", func(t *testing.T) {
		...
		t.Run("AESGCMKW", func(t *testing.T) {
			// let jose generate a key file
			joseJwkFile, joseJwkCleanup, err := jose.GenerateJwk(ctx, t, `{"alg": "A256GCMKW"}`)
			if !assert.NoError(t, err, `jose.GenerateJwk should succeed`) {
				return
			}
			defer joseJwkCleanup()
			...
			t.Run("Encrypt with jose, Decrypt with jwx", func(t *testing.T) {
				...
				// let jwx decrypt the jose crypted file
				payload, err := jwxtest.DecryptJweFile(ctx, joseCryptFile, jwa.A256GCMKW, joseJwkFile)
				if !assert.NoError(t, err, `decryptFile.DecryptJwe should succeed`) {
					// If this fails, let's see what happens if we do this via jose
					joseCryptFile, joseCryptCleanup, err := jose.EncryptJwe(ctx, t, expected, joseJwkFile, true)
					if !assert.NoError(t, err, `jose.EncryptJwe should succeed`) {
			...
			t.Run("Encrypt with jwx, Decrypt with jose", func(t *testing.T) {
				jwxCryptFile, jwxCryptCleanup, err := jwxtest.EncryptJweFile(ctx, expected, jwa.A256GCMKW, joseJwkFile, jwa.A256GCM, jwa.NoCompress)
				if !assert.NoError(t, err, `jwxtest.EncryptJweFile should succeed`) {
					return
				}
				defer jwxCryptCleanup()
		...
		t.Run("PBES2-HS-AESKW", func(t *testing.T) {
			// let jose generate a key file
			joseJwkFile, joseJwkCleanup, err := jose.GenerateJwk(ctx, t, `{"alg": "PBES2-HS512+A256KW"}`)
			if !assert.NoError(t, err, `jose.GenerateJwk should succeed`) {
				return
			}
			defer joseJwkCleanup()
			...
			t.Run("Encrypt with jose, Decrypt with jwx", func(t *testing.T) {
				...
				// let jwx decrypt the jose crypted file
				payload, err := jwxtest.DecryptJweFile(ctx, joseCryptFile, jwa.PBES2_HS512_A256KW, joseJwkFile)
				if !assert.NoError(t, err, `decryptFile.DecryptJwe should succeed`) {
					// If this fails, let's see what happens if we do this via jose
					joseCryptFile, joseCryptCleanup, err := jose.EncryptJwe(ctx, t, expected, joseJwkFile, true)
					if !assert.NoError(t, err, `jose.EncryptJwe should succeed`) {
			...
			t.Run("Encrypt with jwx, Decrypt with jose", func(t *testing.T) {
				jwxCryptFile, jwxCryptCleanup, err := jwxtest.EncryptJweFile(ctx, expected, jwa.PBES2_HS512_A256KW, joseJwkFile, jwa.A256GCM, jwa.NoCompress)
				if !assert.NoError(t, err, `jwxtest.EncryptJweFile should succeed`) {
					return
				}
				defer jwxCryptCleanup()
