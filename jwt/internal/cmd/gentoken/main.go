...
func _main() error {
	...
	for _, pkg := range []string{"jwt", "openid"} {
		if err := genBuilder(pkg); err != nil {
			return errors.Wrapf(err, `failed to generate builder for package %q`, pkg)
		}
	}
...
func genBuilder(pkg string) error {
	var buf bytes.Buffer
	...
	o.LL("func (b *Builder) Build() (Token, error) {")
	o.L("tok := New()")
