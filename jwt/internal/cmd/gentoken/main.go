...
func generateToken(obj *codegen.Object) error {
	...
	o.LL("// NewWith is a convenience function to combine creating a token using %s.New", objectPackage(obj))
	o.L("// and assigning initial claims using %s.Set", objectPackage(obj))
	o.L("func NewWith(options ...ConstructorOption) (%s, error) {", objectInterface(obj))
	o.L("tok := New()")
	o.L("for _, option := range options {")
	o.L("switch option.Ident() {")
	o.L("case identConstructorClaim{}:")
	o.L("pair := option.Value().(*claimPair)")
	o.L("if err := tok.Set(pair.Name, pair.Value); err != nil {")
	o.L("return nil, errors.Wrapf(err, `failed to set claim %%q`, pair.Name)")
	o.L("}")
	o.L("}")
	o.L("}")
	o.L("return tok, nil")
