...
func TestWithBase64Encoder(t *testing.T) {
	...
	t.Run("Roundtrip", func(t *testing.T) {
		...
		signed, err := jwt.Sign(tok, jwt.WithKey(jwa.ES256(), key), jwt.WithBase64Encoder(stdlibbase64.URLEncoding))
		require.NoError(t, err, `jwt.Sign should succeed`)

		parsed := openid.New()
		_, err = jwt.Parse(signed, jwt.WithToken(parsed), jwt.WithKey(jwa.ES256(), key.PublicKey), jwt.WithBase64Encoder(stdlibbase64.URLEncoding))
		require.NoError(t, err, `jwt.Parse should succeed`)
	...
	t.Run("Contributed Test Case", func(t *testing.T) {
		...
		_, err = jwt.Parse(buf.Bytes(), jwt.WithBase64Encoder(stdlibbase64.URLEncoding), jwt.WithKey(jwa.ES256(), key))
		require.NoError(t, err, `jwt.Parse should succeed`)
	})
}
