...
* [jwe] `x5c` field type has been changed to `*cert.Chain` instead of `[]string`

* [jwe] Method signature for `jwe.Parse()` has been changed to include options,
  but options are currently not used

* [jwe] `jwe.ReadFile` now supports the option `jwe.WithFS` which allows you to
  read data from arbitrary `fs.FS` objects

* [jwe] jwe.WithKeyUsed has been added to allow users to retrieve
...
* [jwk] `jwk.Fetcher` and `jwk.FetchFunc` has been added.
  They represent something that can fetch a `jwk.Set`

* [jwk] `jwk.CertificateChain` has been removed, use `*cert.Chain`
* [jwk] `x5c` field type has been changed to `*cert.Chain` instead of `[]*x509.Certificate`

* [jwk] `jwk.ReadFile` now supports the option `jwk.WithFS` which allows you to
  read data from arbitrary `fs.FS` objects

* [jwk] Added `jwk.PostFetcher`, `jwk.PostFetchFunc`, and `jwk.WithPostFetch` to
...
* [jwk] Reworked `jwk.AutoRefresh` in terms of `github.com/lestrrat-go/httprc`
...
* [jwk] Reimplemented `jwk.Fetch` in terms of `github.com/lestrrat-go/httprc`.

* [jwk] Previously `jwk.Fetch` and `jwk.AutoRefresh` respected backoff options,
...
* [jws] `x5c` field type has been changed to `*cert.Chain` instead of `[]string`

* [jws] `jws.ReadFile` now supports the option `jws.WithFS` which allows you to
  read data from arbitrary `fs.FS` objects
...
* [jwt] `jwt.ReadFile` now supports the option `jwt.WithFS` which allows you to
  read data from arbitrary `fs.FS` objects
