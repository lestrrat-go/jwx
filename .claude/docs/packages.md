...
- Sub-packages: `jwk/ecdsa` — elliptic curve registration (`RegisterCurve(alg, curve, PointValidator)`, `CurveFromAlgorithm`, `AlgorithmFromCurve`, `ValidatorFromCurve`, `PointValidator` interface, `PointValidatorFunc` adapter); `jwk/jwkbb` — X.509/PEM encoding building blocks: primitives `EncodeX509(dst, v)` / `DecodeX509(block)`, plus the decoder+encoder registries (`X509Decoder` / `X509DecodeFunc` / `RegisterX509Decoder(ident, d) error` / `UnregisterX509Decoder(ident)`, and the symmetric encoder side `X509Encoder` / `X509EncodeFunc` / `RegisterX509Encoder(ident, e) error` / `UnregisterX509Encoder(ident)`). `X509Decoders() iter.Seq[X509Decoder]` / `X509Encoders() iter.Seq[X509Encoder]` expose snapshot iterators over the registrations. `EncodePEM(keys ...any) ([]byte, error)` runs each raw key through the encoder chain and concatenates PEM blocks; decode dispatch is driven by `jwk.ParseKey` with `jwk.WithPEM(true)`. `jwk/jwkunsafe` — low-level key constructors (`NewKey`, `NewPublicKey`) for extension modules
