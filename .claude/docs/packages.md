...
- Sub-package: `jws/internal/jwsbb` — sign-side-only helper shared by `jws` and `jws/jwsbb`. `RequireECDSACurve` rejects a key whose curve disagrees with the ES256/ES384/ES512 algorithm per RFC 7518 Section 3.4. Called from `jws/jwsbb`'s `dispatchECDSASign` and from `jws.signStreaming`, covering `jws.Sign`/`jwt.Sign` (including its fast path) and the streaming detached-payload path. `jws.Verify`'s key-to-algorithm inference is deliberately untouched and stays permissive.
