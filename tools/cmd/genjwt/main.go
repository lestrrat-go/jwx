...
func generateToken(obj *codegen.Object) error {
	...
	o.WriteImportPkgs(
		codegen.ImportPkg{
			Alias: `jwterrs`,
			URL:   `github.com/lestrrat-go/jwx/v3/jwt/internal/errors`,
		},
	)
	...
	for _, f := range fields {
		...
		if f.Name(false) == `algorithm` {
			...
			o.L("t.algorithm = &tmp")
			o.L("default:")
			o.L("return jwterrs.ErrFromClaim(`invalid value for %%s key: %%T`, %s, value)", keyName)
			o.L("}")
			o.L("return nil")
			...
			o.L("return jwterrs.ErrFromClaim(`invalid value for %%s key: %%w`, %s, err)", keyName)
	...
	o.L("if err != nil {")
	o.L("return jwterrs.ErrFromParse(jwterrs.PrefixTokenUnmarshalJSON, `error reading token: %%w`, err)")
	o.L("}")
	...
	o.L("return jwterrs.ErrFromParse(jwterrs.PrefixTokenUnmarshalJSON, `expected '%%c', but got '%%c'`, tokens.OpenCurlyBracket, tok)")
	o.L("}")
	...
	for _, f := range fields {
		if f.Type() == "string" {
			...
			o.L("return jwterrs.ErrFromParse(jwterrs.PrefixTokenUnmarshalJSON, `failed to decode value for key %%s: %%w`, %sKey, err)", f.Name(true))
			o.L("}")
	...
	o.L("continue")
	o.L("}")
	o.L("return jwterrs.ErrFromParse(jwterrs.PrefixTokenUnmarshalJSON, `could not decode field %%s: %%w`, tok, err)")
	o.L("}")
	o.L("default:")
	o.L("return jwterrs.ErrFromParse(jwterrs.PrefixTokenUnmarshalJSON, `invalid token %%T`, tok)")
	o.L("}")
	o.L("}")
	...
	for _, f := range fields {
		...
		if f.Name(false) == `audience` {
			...
			o.L("if err != nil {")
			o.L("return nil, jwterrs.ErrFromClaim(`failed to encode \"aud\": %%w`, err)")
			o.L("}")
			...
			o.L("if err != nil {")
			o.L("return nil, jwterrs.ErrFromClaim(`failed to encode %q: %%w`, err)", f.JSON())
			o.L("}")
			o.L("pairs = append(pairs, claimPair{Name: %sKey, Value: buf})", f.Name(true))
		} else if f.Type() == "[]byte" {
			...
			o.L("if err != nil {")
			o.L("return nil, jwterrs.ErrFromClaim(`failed to encode field %q: %%w`, err)", f.JSON())
			o.L("}")
			o.L("pairs = append(pairs, claimPair{Name: %sKey, Value: buf})", f.Name(true))
		}
		o.L("}")
	...
	o.L("if err != nil {")
	o.L("return nil, jwterrs.ErrFromClaim(`failed to encode field %%q: %%w`, k, err)")
	o.L("}")
	...
	o.L("if err != nil {")
	o.L("return nil, jwterrs.ErrFromClaim(`failed to make pairs: %%w`, err)")
	o.L("}")
