...
func generateHeaders(obj *codegen.Object) error {
	...
	for _, f := range obj.Fields() {
		o.L("case %sKey:", f.Name(true))
		o.L("if h.%s == nil {", f.Name(false))
		o.L("return errFromField(`field %%q not found`, name)")
		o.L("}")
		...
		o.R("); err != nil {")
		o.L("return errFromField(`failed to assign value for field %%q: %%w`, name, err)")
		o.L("}")
	}
	o.L("default:")
	...
	for _, f := range obj.Fields() {
		...
		if f.Bool(`hasAccept`) {
			...
			o.L("return errFromField(`invalid value for %%s key: %%w`, %sKey, err)", f.Name(true))
			...
			if f.Name(false) == "contentEncryption" {
				...
				o.L("return errFromField(`%#v field cannot be an empty string`)", f.JSON())
				o.L("}")
			}
			...
			o.L("return errFromField(`invalid value for %%s key: %%T`, %sKey, value)", f.Name(true))
		}
	}
	o.L("default:")
	...
	o.L("if err != nil {")
	o.L("return errFromParse(prefixParse, `error reading token: %%w`, err)")
	o.L("}")
	o.L("switch tok := tok.(type) {")
	o.L("case json.Delim:")
	o.L("// Assuming we're doing everything correctly, we should ONLY")
	o.L("// get either tokens.OpenCurlyBracket or tokens.CloseCurlyBracket here.")
	o.L("if tok == tokens.CloseCurlyBracket { // End of object")
	o.L("break LOOP")
	o.L("} else if tok != tokens.OpenCurlyBracket {")
	o.L("return errFromParse(prefixParse, `expected '%%c' but got '%%c'`, tokens.OpenCurlyBracket, tok)")
	o.L("}")
	...
	for _, f := range obj.Fields() {
		if f.Type() == "string" {
			...
			o.L("return errFromParse(prefixParse, `failed to decode value for key %%s: %%w`, %sKey, err)", f.Name(true))
			o.L("}")
		} else if f.Type() == "[]byte" {
			o.L("case %sKey:", f.Name(true))
			...
			o.L("if err != nil {")
			o.L("return errFromParse(prefixParse, `failed to parse JWK for key %%s: %%w`, %sKey, err)", f.Name(true))
			o.L("}")
	...
	o.L("}")
	o.L("default:")
	o.L("return errFromParse(prefixParse, `invalid token %%T`, tok)")
	o.L("}")
	o.L("}")
	...
	o.L("return nil, errFromField(`failed to encode value for field %%s`, k)")
	o.L("}")
